#!/usr/bin/env node
/**
 * lpictl - CLI tool for LPI operations
 *
 * Commands:
...
import { LCE } from '../../packages/node-lpi/src/types';
import { validateLCE } from '../../packages/node-lpi/src/validator';
import {
  encodeLCE,
  decodeLCE,
  compareSizes,
} from '../../packages/node-lpi/src/cbor';
import { sign, verify, generateKeys } from '../../packages/node-lpi/src/ltp';
...
program
  .name('lpictl')
  .description('CLI tool for LPI (Liminal Presence Interface) operations')
  .version('0.2.0');
...
program
  ...
  .action(async (outputDir: string = '.') => {
    try {
      ...
      const privateKeyPath = path.join(outputDir, 'lpi-private.key');
      const publicKeyPath = path.join(outputDir, 'lpi-public.key');
