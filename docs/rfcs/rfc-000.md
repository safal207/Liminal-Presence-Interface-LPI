# RFC-000: The Liminal Presence Interface (LPI) — Overview

**Status:** Draft
**Author(s):** LPI Contributors
**Created:** 2025-01-15
**Updated:** 2025-01-15
**Version:** 0.1.0
...
The **Liminal Presence Interface (LPI)** is a semantic communication protocol that operates as **Layer 8** above the traditional OSI Layer 7 (Application). LPI enriches application-level messages with contextual metadata including **intent** (what the message aims to achieve), **affect** (emotional context), **consent** (privacy policy), **coherence** (quality metrics), **trust** (cryptographic proofs), and **memory** (session continuity).

LPI enables human-AI systems to communicate with semantic clarity, emotional awareness, and explicit consent, addressing the gap between raw data transmission and meaningful interaction.
...
LPI introduces a **semantic layer** that sits above application protocols, wrapping messages with rich contextual metadata while remaining **protocol-agnostic** and **backward-compatible**.
...
LPI is built on five core principles:
...
LPI explicitly **does not**:

1. **Replace existing protocols** - LPI wraps HTTP/WS/gRPC, doesn't replace them
2. **Define application logic** - LPI provides context, not business rules
3. **Guarantee AI behavior** - LPI helps AI understand intent, doesn't control responses
4. **Handle encryption** - Use TLS; LPI adds semantic layer, not transport security
5. **Store data** - LPI defines session context, not persistence layer
6. **Enforce consent** - LPI signals policy, application must enforce
7. **Compete with REST/GraphQL** - LPI is orthogonal to API design patterns
...
The data structure that wraps messages with semantic metadata. The fundamental unit of LPI.

### 5.2 Subsystems

**LHS (Liminal Handshake Sequence)**
Protocol for establishing LPI-aware connections (primarily for WebSocket). Steps: Hello → Mirror → Bind → Seal → Flow.
...
```
┌─────────────────────────────────────────────────────────────┐
│  Layer 8: LPI (Semantic/Context Layer)                      │
...
| Layer | Traditional | With LPI |
|-------|-------------|----------|
| 8 | — | **LPI** (intent, affect, consent) |
...
3. **Optional adoption** - Clients without LPI still work (graceful degradation)
...
LPI integrates with existing Layer 7 protocols through:
...
The LCE is the core data structure of LPI:
...
LPI supports two encoding modes:
...
**Purpose:** Establish LPI-aware WebSocket connections with negotiated capabilities.
...
1. **LPI-aware client + LPI-aware server** → Full LPI features
2. **LPI-aware client + Legacy server** → Client sends LCE, server ignores
3. **Legacy client + LPI-aware server** → Server provides defaults
4. **Legacy client + Legacy server** → Normal operation (no LPI)
...
LPI is **orthogonal** to REST/GraphQL:
...
**LPI Version Format:** `MAJOR.MINOR.PATCH`
...
- **Website:** https://lpi.dev (coming soon)
- **GitHub:** https://github.com/safal207/Liminal-Presence-Interface-LPI
- **Schema:** https://lpi.dev/schema/lce-v0.1.json
- **Docs:** https://lpi.dev/docs
...
A: No. LPI wraps existing APIs. Your endpoints remain unchanged.

**Q: Is LPI only for AI?**
A: No, but it's optimized for human-AI. Human-human chat benefits too.

**Q: How do I get started?**
A: Install SDKs (`npm install node-lpi` or `pip install python-lpi`), add middleware, start using LCE.
