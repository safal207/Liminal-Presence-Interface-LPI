...
- Ed25519 (-8) is the only COSE algorithm currently supported; broader algorithm agility and key management remain future work.【F:packages/node-lri/src/cbor/lce-cose.ts†L272-L307】【F:packages/python-lri/lri/cbor_cose.py†L185-L206】
- Canonical encoding relies on in-memory normalization. Streaming serializers or chunked signing are not yet available, so large payloads may incur additional allocations.【F:packages/node-lri/src/cbor/lce-cose.ts†L70-L219】【F:packages/python-lri/lri/cbor_cose.py†L35-L150】
...
- Validation currently asserts the presence of required LCE fields but stops short of full schema enforcement. Integrating the shared validator into the signing pipeline would give earlier feedback on malformed payloads.【F:packages/node-lri/src/cbor/lce-cose.ts†L17-L35】【F:packages/python-lri/lri/cbor_cose.py†L176-L206】
