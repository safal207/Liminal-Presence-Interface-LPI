...
```python
@app.exception_handler(HTTPException)
async def format_lpi_errors(_, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={"detail": exc.detail})
```
...
For advanced features, please use the [Node.js SDK](../node-lpi/) (v0.2.0).
...
- [Node SDK](../node-lpi/) - For advanced features
