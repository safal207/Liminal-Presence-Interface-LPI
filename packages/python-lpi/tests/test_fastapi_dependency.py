...
@pytest.fixture()
def fastapi_dependency_app() -> tuple[LPI, TestClient]:
    """Create a FastAPI app configured with the LPI dependency."""
    ...
    async def ingest_endpoint(
        payload: dict, lce: LCE = Depends(lpi.dependency(required=True))
    ) -> dict[str, str]:
        return {"intent": lce.intent.type, "echo": payload.get("message", "")}

    @app.get("/respond")
    async def respond_endpoint(
        response: Response, lce: Optional[LCE] = Depends(lpi.dependency())
    ) -> dict[str, Optional[str]]:
        response_lce = LCE(
            v=1,
        ...
        response.headers[lpi.header_name] = lpi.create_header(response_lce)
        return {
            "request_intent": lce.intent.type if lce else None,
            "response_intent": response_lce.intent.type,
        }

    return lpi, TestClient(app)
...
def test_dependency_accepts_valid_requests(
    ...
    lpi, client = fastapi_dependency_app

    headers = {}
    if header_factory:
        headers[lpi.header_name] = header_factory(lpi)
...
def test_required_dependency_rejects_missing_header(fastapi_dependency_app):
    ...
    assert response.json() == {
        "detail": {"error": "LCE header required", "header": lpi.header_name}
    }
...
def test_dependency_returns_422_errors(
    ...
    response = client.post(
        "/ingest",
        json={"message": "ping"},
        headers={lpi.header_name: header},
    )
...
def test_dependency_round_trips_response_headers(fastapi_dependency_app):
    lpi, client = fastapi_dependency_app
    request_header = lpi.create_header(
    ...
    encoded_header = response.headers[lpi.header_name]
