"""Tests for LPI FastAPI integration"""

import base64
import json
import pytest
from fastapi import HTTPException, Request
from lpi.lpi import LPI
...
class TestLPIInit:
    """Tests for LPI initialization"""

    def test_default_initialization(self):
        """LPI should initialize with defaults"""
        lpi = LPI()
        assert lpi.header_name == "LCE"
        assert lpi.validate is True

    def test_custom_header_name(self):
        """LPI should accept custom header name"""
        lpi = LPI(header_name="X-Custom-LCE")
        assert lpi.header_name == "X-Custom-LCE"

    def test_validation_can_be_disabled(self):
        """LPI should allow disabling validation"""
        lpi = LPI(validate=False)
        assert lpi.validate is False


class TestLPICreateHeader:
...
class TestLPIParseRequest:
    ...
    async def test_parse_valid_lce(self):
        ...
        result = await lpi.parse_request(request)
    ...
    async def test_parse_request_no_header_not_required(self):
        ...
        result = await lpi.parse_request(request, required=False)
    ...
    async def test_parse_request_no_header_required(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await lpi.parse_request(request, required=True)
    ...
    async def test_parse_request_malformed_base64(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await lpi.parse_request(request)
...
class TestLPIRoundTrip:
    ...
    async def test_round_trip_minimal(self):
        ...
        # Encode
        header = LPI.create_header(original)
